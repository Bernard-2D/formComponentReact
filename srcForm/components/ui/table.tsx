...
const Table = React.forwardRef<HTMLTableElement, TableProps>(
  ({ className, contentClassName, isEmpty, ...props }, ref) => (
    <div className={cn('relative w-full', contentClassName)}>
      ...
      {isEmpty && (
        <div className="text-light-500 mt-20 flex w-full flex-col items-center">
          <img src={Empty} alt="" />
          暂无内容
        </div>
      )}
...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn('border-b bg-white transition-colors hover:bg-minor-6 data-[state=selected]:bg-main-2', className)}
      {...props}
    />
  )
)
